...
namespace TileCook.API.Controllers
{
    public class TileJSONController : ApiController
    {
        ...
        public HttpResponseMessage GetTileJSON(string layer)
        {
            ...
            {
                tilejson.grids = new List<string>() { WebUtility.UrlDecode(Url.Link("TMSTile", new { Version = "1.0.0", TileMap = tileLayer.Name, Z = "{z}", X = "{x}", Y = "{y}", Format = "json" })) };
            }

            // Set tile endpoint
            tilejson.tiles = new List<string>() { WebUtility.UrlDecode(Url.Link("TMSTile", new { Version = "1.0.0", TileMap = tileLayer.Name, Z = "{z}", X = "{x}", Y = "{y}", Format = format })) };
